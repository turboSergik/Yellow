macro(set_conan_params)
    ...
    elseif (${CMAKE_CXX_COMPILER_ID} STREQUAL "MSVC")
        ...
        if (IS_DEFAULT EQUAL -1)
            ...
            string(SUBSTRING ${MSVC_VERSION} 0 4 MSVC_VERSION)
            if (${MSVC_VERSION} STREQUAL "19.2")
                set(MSVC_VERSION "16")
            elseif(${MSVC_VERSION} STREQUAL "19.1")
                set(MSVC_VERSION "15")
            else()
                string(SUBSTRING MSVC_VERSION 0 2 MSVC_VERSION)
                if (${MSVC_VERSION} STREQUAL "19")
                    set(MSVC_VERSION "14")
                elseif (${MSVC_VERSION} STREQUAL "18")
                    set(MSVC_VERSION "12")
                elseif (${MSVC_VERSION} STREQUAL "17")
                    set(MSVC_VERSION "11")
                endif()
            endif()
            string(APPEND CONAN_PARAMS ${MSVC_VERSION})
            # maybe need runtime
    ...
    elseif (${CMAKE_CXX_COMPILER_ID} STREQUAL "AppleClang")
        execute_process(COMMAND conan profile show default OUTPUT_VARIABLE CONAN_PROFILE)
        string(FIND ${CONAN_PROFILE} "compiler=apple_clang" IS_DEFAULT)
        if (IS_DEFAULT EQUAL -1)
            string(APPEND CONAN_PARAMS "apple_clang -s compiler.version=")
